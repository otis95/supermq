...
type (
	...
	OnPublishFunc  func(msg *message.PublishMessage, fromRoute bool) error
)
...
func (this *service) start() error {
	...
	// If this is a server
	if !this.client && this.route == nil {
		// Creat the onPublishFunc so it can be used for published messages
		this.onpub = func(msg *message.PublishMessage, fromRoute bool) error {
		...
		this.onpub = func(msg *message.PublishMessage, fromRoute bool) error {
			//如果msg来自于route，就需要在route中再次pulish到route了
			if fromRoute {
				return nil
			}
			if err := this.publish(msg, nil); err != nil {
...
func (this *service) stop() {
	...
	if !this.client && this.route == nil && this.sess.Cmsg.WillFlag() {
	...
	if this.route == nil && this.sess.Cmsg.CleanSession() && this.sessMgr != nil {
	...
	if this.route != nil {
		this.server.mu.Lock()
		if this.route == this.server.routes[this.route.remoteID] {
			delete(this.server.routes, this.route.remoteID)
		}
		delete(this.server.route_client, this.id)
		this.server.mu.Unlock()
		this.clearPingTimer()
		for t, _ := range this.topics {
			if err := this.topicsMgr.Unsubscribe([]byte(t), &this.onpub); err != nil {
				glog.Errorf("(%s): Error unsubscribing topic %q: %v", this.cid(), t, err)
			}
		}
