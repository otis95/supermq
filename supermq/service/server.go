...
func (this *Server) ListenAndServeTLS(uri, certFile, keyFile string) error {
	...
	//glog.Info("server/ListenAndServeTLS Scheme : ", u.Scheme)
	//glog.Info("server/ListenAndServeTLS Host: ", u.Host)
...
func (this *Server) Close() error {
	...
	if len(this.route_client) != 0 {
		for _, svc := range this.route_client {
			svc.stop()
		}
	}
...
func (this *Server) handleConnection(c io.Closer) (svc *service, err error) {
	...
	glog.Infof("(%s) server/handleConnection: Connection established (%s).", svc.cid(), conn.RemoteAddr())
